...
export default function SplitExpenseScreen({ navigation, route }: any) {
  ...
  const calculateSplitAmounts = () => {
    ...
    try {
      const customValues = formData.splitType === 'custom' 
        ? Object.fromEntries(
            Object.entries(formData.customAmounts).map(([id, amount]) => [id, parseFloat(amount) || 0])
          )
        : undefined;

      return calculateSplit(amount, formData.splitType, participants, customValues);
    } catch (error) {
      return [];
    }
  };
  ...
  const handleSaveExpense = async () => {
    if (!formData.amount || !formData.description || !formData.paidBy || formData.splitBetween.length === 0) {
    ...
    try {
      ...
      const validation = validateSplit(
        ...
        formData.splitBetween.map(id => ({
          userId: id,
          userName: trip?.participants?.find(p => p.id === id)?.name || 'Unknown',
          amount: 0,
          isPaid: false,
          settlementStatus: 'pending' as const,
        })),
  ...
  const isValidSplit = Math.abs(totalSplit - parseFloat(formData.amount || '0')) < 0.01;
        ...
        <View style={styles.form}>
                    ...
                    <View style={styles.amountInput}>
                      ...
                      {formData.splitType === 'equal' && (
                        <Text style={styles.equalAmount}>
                          ${splitAmounts.find(s => s.userId === participant.id)?.amount.toFixed(2) || '0.00'}
                        </Text>
                      )}
                    </View>
                  )}
          ...
          {formData.splitType !== 'equal' && (
            <View style={styles.summary}>
              ...
              {!isValidSplit && (
                <Text style={styles.errorText}>Split amounts don't match total</Text>
              )}
            </View>
          )}
        </View>
